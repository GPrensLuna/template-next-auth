...
export const getFetchWithCancel = async <ResponseType, RequestType = any>(
	...
	timeout?: number,
): Promise<ResponseDTO<ResponseType> | undefined> => {
	...
	try {
		...
		const response = await fetch(`${process.env.NEXT_PUBLIC_URL_FRONT}${url}`, {
			method,
		...
		//? Optionally check for message in the response, if it exists
		const message = (responseData as any)?.message || "Response received successfully";

		//* Cache the response if not using "no-store"
		if (cacheMode !== "no-store") {
			cache.set(url, { data: responseData, message });
		}
		...
		if (cacheMode !== "no-store" && cache.has(url)) {
			return cache.get(url) as ResponseDTO<ResponseType>;
		}
